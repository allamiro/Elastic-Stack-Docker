...
Adds a TLS-secured Kafka broker with dual Logstash pipeline for event streaming. Data flows: `logstash-kafka-in` → `Kafka` → `logstash-kafka-out` → `Elasticsearch`. The broker certificate is generated by the setup service and chained to the stack CA, so every client reuses the shared `certs` volume.
